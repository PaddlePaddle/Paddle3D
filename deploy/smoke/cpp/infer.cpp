...
DEFINE_int32(trt_precision, 0, "Precision type of tensorrt, 0: kFloat32, 1: kInt8, 2: kHalf");
...
void get_image(const std::string& image, float* data) {
    ...
    for(int i = 0; i < 3; ++i) {
        rgbChannels[i].convertTo(rgbChannels[i], CV_32FC1, 1 / std_values[i], (0.0 - mean_values[i]) / std_values[i]);
    }
...
std::shared_ptr<Predictor> InitPredictor() {
    ...
    if (FLAGS_collect_dynamic_shape_info) {
        ...
        config.EnableTensorRtEngine(1 << 30, 1, 5, PrecisionType(FLAGS_trt_precision), false, false);
...
void run(Predictor *predictor,
         const std::vector<float> &input_im_data,
    ...
    int input_num = std::accumulate(input_im_shape.begin(), input_im_shape.end(), 1,
                                    std::multiplies<int>());
    ...
    int outsize = std::accumulate(outshape.begin(), outshape.end(), 1, std::multiplies<int>());
...
int main(int argc, char *argv[]) {
    ...
    std::vector<float> out_data;
    run(predictor.get(),
        input_im_data, input_im_shape,
        input_K_data, input_K_shape,
        input_ratio_data, input_ratio_shape,
        out_data);
