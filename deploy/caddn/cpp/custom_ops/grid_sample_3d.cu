...
static __forceinline__ __device__ bool InBounds3D(int64_t d, int64_t h, int64_t w, int64_t D, int64_t H, int64_t W) {
...
template <typename T, typename index_t>
  __global__ void GridSample3DCudaKernel(
      ...
      bool align_corners) {
    //printf("size: %d, %d, %d, %d, %d, %d \n", out_c, out_d, out_w, out_h, in_d, in_w);
    index_t inp_sW = 1;
    index_t inp_sH = in_w;
    ...
    CUDA_KERNEL_LOOP_TYPE(index, nthreads, index_t) {
      ...
      if (interpolation_mode == Mode::bilinear) {
        ...
        for (index_t c = 0; c < out_c; ++c, inp_ptr_NC += inp_sC, out_ptr_NCDHW += out_sC) {
...
std::vector<paddle::Tensor> GridSample3DCUDAForward(const paddle::Tensor& x, 
    ...
    auto output = paddle::full({n, c, out_d, out_h, out_w}, 0, paddle::DataType::FLOAT32, paddle::GPUPlace());
    ...
    if( error_check != cudaSuccess ){ printf("%s\n" , cudaGetErrorString( error_check ) ); }
    // printf("size: %d, %d, %d, %d, %d, %d \n", n, c, out_d, out_h, count, block_num);
    return {output};
}
  ...
  __global__ void GridSample3DCudaBackwardKernel(
    ...
    CUDA_KERNEL_LOOP_TYPE(index, nthreads, index_t) {
      ...
      if (mode == Mode::bilinear) {
        ...
        index_t gOut_offset = n * gOut_sN + d * gOut_sD + h * gOut_sH + w * gOut_sW;
        index_t inp_offset_NC = n * inp_sN;
        T* gInp_ptr_NC = grad_input + n * inp_sN;
        for (index_t c = 0; c < out_c; ++c, gOut_offset += gOut_sC, gInp_ptr_NC += inp_sC, inp_offset_NC += inp_sC) {
        ...
        if (grad_grid != nullptr) {
          ...
          index_t gOut_offset =  n * gOut_sN + d * gOut_sD + h * gOut_sH + w * gOut_sW;
          T* gInp_ptr_NC = grad_input + n * inp_sN;
          for (index_t c = 0; c < out_c; ++c, gOut_offset += gOut_sC, gInp_ptr_NC += inp_sC) {
...
std::vector<paddle::Tensor> GridSample3DCUDABackward(const paddle::Tensor& x, 
                                                        const paddle::Tensor& grid,
    ...
    auto grid_grad_output = paddle::empty({n, out_d, out_h, out_w, 3}, paddle::DataType::FLOAT32, paddle::GPUPlace());
    auto x_grad_output = paddle::full({n, c, in_d, in_h, in_w}, 0, paddle::DataType::FLOAT32, paddle::GPUPlace());
    ...
    int block_num = (count - 1) / max_threads_per_block + 1;
              ...
              GridSample3DCudaBackwardKernel<float, int>
                ...
                in_h,
                in_w,
                x_grad_output.data<float>(),
                grid_grad_output.data<float>(),
                enum_mode,
                enum_padding_mode,
                align_corners);
