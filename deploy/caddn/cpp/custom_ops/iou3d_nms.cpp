...
int boxes_overlap_bev_gpu(paddle::Tensor boxes_a, paddle::Tensor boxes_b, paddle::Tensor ans_overlap){
...
int boxes_iou_bev_gpu(paddle::Tensor boxes_a, paddle::Tensor boxes_b, paddle::Tensor ans_iou){
    // params boxes_a: (N, 7) [x, y, z, dx, dy, dz, heading]
    // params boxes_b: (M, 7) [x, y, z, dx, dy, dz, heading]
    // params ans_overlap: (N, M)
    CHECK_INPUT(boxes_a);
...
std::vector<paddle::Tensor> nms_gpu(const paddle::Tensor& boxes, float nms_overlap_thresh){
    ...
    auto keep = paddle::empty({boxes.shape()[0]}, paddle::DataType::FLOAT32, paddle::CPUPlace());
    auto num_to_keep_tensor = paddle::empty({1}, paddle::DataType::FLOAT32, paddle::CPUPlace());
...
int nms_normal_gpu(paddle::Tensor boxes, paddle::Tensor keep, float nms_overlap_thresh){
    // params boxes: (N, 7) [x, y, z, dx, dy, dz, heading]
    // params keep: (N)
