...
void run(Predictor *predictor, 
  ...
  for (int i = 0; i < 100; i++) {
    ...
    auto tt = std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time)
                .count() / 1000000.0;
...
void resize(const cv::Mat & img, cv::Mat & resize_img, int resized_h, int resized_w)
{
  cv::resize(img, resize_img, cv::Size(resized_h, resized_w),0, 0, cv::INTER_LINEAR);
}
...
int main(int argc, char *argv[]) {
  ...
  std::vector<float> cam_data{7.183351e+02, 0.000000e+00, 6.003891e+02, 4.450382e+01, 
  ...
  std::vector<float> lidar_data= {0.0048523, -0.9999298, -0.01081266, -0.00711321,
  ...
  std::vector<float> labels;
  std::vector<float> scores;
  run(predictor.get(), images_shape, input_data, 
                       cam_shape, cam_data,
