...
from utils import Calibration, show_lidar_with_boxes, total_imgpred_by_conf_to_kitti_records, \
        ...
        for idx, sample in logger.enumerate(self.eval_dataloader, msg=msg):
            ...
            if mode == 'pcd':
                for result in results:
                    ...
                    calib = Calibration(result['path'].replace('velodyne', 'calib').replace('bin', 'txt'))
            ...
            if mode == 'image':
                for result in results:
                    kitti_records = total_imgpred_by_conf_to_kitti_records(result, 0.3)
            ...
            if mode == 'bev':
                for result in results:
                    ...
                    show_bev_with_boxes(pc_velo, result['bboxes_3d'], result['confidences'], calib)
