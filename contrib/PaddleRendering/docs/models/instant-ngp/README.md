...
Instant-NGP
以提升神经渲染模型的训练、推理性能为出发点，提出使用包含可学习参数的多分辨率哈希编码结构取代 [NeRF](https://arxiv.org/abs/2003.08934)
系列模型中使用的位置编码，使得模型可以使用更小的多层感知机（MLP）。同时该模型使用了作者在 [Real-time Neural Radiance Caching for Path Tracing](https://tom94.net/data/publications/mueller21realtime/mueller21realtime.pdf)
中提出的 Fully Fused MLP，将神经渲染模型的训练耗时从小时级缩短到分钟级，并实现了实时的渲染速度。

## <h2 id="3">模型库</h2>

- Instant-NGP 在 NeRF Synthetic 数据集上的表现
...
我们提供了在开源数据集上的训练配置与结果，详见 [Instant-NGP训练配置](../../../configs/instant-ngp)。

## <h2 id="5">使用教程</h2>

### <h3 id="51">数据准备</h3>

数据文件软链至`PaddleRender/data/`目录下，或在 [配置文件](../../../configs/instant-ngp) 中指定数据集路径。
