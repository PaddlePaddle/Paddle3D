...
__all__ = [
    "near_far_from_aabb", "near_far_from_sphere", "grid_query", "contract", "contract_inv",
    "transmittance_from_alpha", "unpack_info", "ray_marching",
    "pdf_ray_marching", "render_visibility", "pack_info",
    "render_weight_from_density", "accumulate_along_rays", "render_alpha_from_densities", "render_alpha_from_sdf"
]
...
def ray_marching(origins: paddle.Tensor,
    ...
    if nears is None or fars is None:
        if scene_aabb is not None:
            ...
            assert(aabb is None)
            nears, fars = near_far_from_sphere(origins, directions, aabb)            
        else:
...
def get_cosine_coeff(ray_samples: RaySamples = None,
    ...
    cos_val = (next_sdf - prev_sdf) / (next_z_vals - prev_z_vals + 1e-5).squeeze(axis=-1) # [b, n_samples]
    prev_cos_val = paddle.concat([paddle.zeros([batch_size, 1]), cos_val[:, :-1]], axis=-1) # [b, n_samples]
...
def render_alpha_from_sdf(ray_samples, signed_distances, inv_s, coeff=1.0, clip_alpha=True):
...
def render_alpha_from_densities(ray_samples: RaySamples = None,
                                densities: paddle.Tensor = None) -> paddle.Tensor:
...
def render_weights_from_alpha(alpha):
    ...
    weights = alpha * paddle.cumprod(
         paddle.concat([paddle.ones([batch_size, 1, 1]), 1. - alpha + 1e-7], 1), 1)[:, :-1, :]
    return weights
