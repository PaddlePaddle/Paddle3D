...
// #define CHECK_INPUT(x) PD_CHECK(x.place() == paddle::PlaceType::kGPU, #x "
// must be a GPU Tensor.") #define CHECK_INPUT(x) PD_CHECK(x.is_gpu(), #x " must
// be a GPU Tensor.")
 ...
 *@param[in] input_distortion_coeffs     畸变系数，必须为6，顺序为k1 k2 k3 k4 p1
 *p2
 ...
 *
 *@param[in] xy_coords                   输入的xy坐标（相机坐标系下）Nx2
 *@param[in] input_distortion_coeffs     畸变系数，必须为6个，顺序为k1 k2 k3 k4
 *p1 p2
 *@param[in] max_iterations     迭代次数
 *@param[in] eps     最小分辨误差
