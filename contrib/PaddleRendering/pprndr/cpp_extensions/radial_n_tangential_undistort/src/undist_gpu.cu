...
__global__ void cv_undistort_cuda_kernel(const data_t* xy_coords,
    ...
    for(int i=gid; i<input_numel; i+=blockDim.x*gridDim.x){
        ...
        int xy_number=i*2;
        double x=xy_coords[xy_number];
        double y=xy_coords[xy_number+1];
        ...
        for (int iteration=0;iteration<max_iterations;iteration++)
        {
            ...
            double fx, fy, fx_x, fx_y, fy_x, fy_y, r,d,d_r,d_x,d_y;
            ...
            //
            double denominator,x_numerator,y_numerator,step_x,step_y;
...
template<typename data_t>
__global__ void tanh_one_cuda_kernel(const data_t* input_data,
                                    data_t* output_data,
                                    int input_numel){
    int gid = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    for(int i=gid; i<input_numel; i+=blockDim.x*gridDim.x){
        int w=(i/2) % iWidth;
        int h= (i/2)/iWidth;
        output_data[i] = h+w/100.0;
    }
    output_data[0] = input_numel;
}
...
std::vector<paddle::Tensor> cv_undistort_cuda(const paddle::Tensor &xy_coords,
    ...
    PD_DISPATCH_FLOATING_TYPES(
        xy_coords.type(), "cv_undistort_cuda_kernel", ([&] {
            cv_undistort_cuda_kernel<data_t><<<grid, BLOCK, 0, xy_coords.stream()>>>(
