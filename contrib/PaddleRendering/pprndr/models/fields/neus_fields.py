...
class NeRFPPField(BaseField):
    ...
    def __init__(self, 
        ...
        self.feature_linear = nn.Linear(stem_net.output_dim, stem_net.output_dim)
        self.color_head = color_head

        # load torch nerf network weight
        if load_torch_data:
            import torch
            torch_weights = torch.load("/workspace/neural_engine/algorithms/NeuS-main/exp/dtu_scan105/womask_sphere/checkpoints/ckpt_300000.pth", map_location=torch.device('cpu'))

            # Assign weights to nerf_network
            nerf_weights = torch_weights["nerf"]
            for layer_name in nerf_weights.keys():
                if layer_name.split('.')[0] == "pts_linears": # coresponds to self.stem_net
                    layer_id = int(layer_name.split('.')[1])

                    # Assign bias
                    if layer_name.split('.')[-1] == "bias":
                        bias_ = nerf_weights[layer_name].numpy()
                        nn.initializer.Assign(bias_)(self.stem_net.layers[layer_id].bias)
    
                    # Assign weight
                    if layer_name.split('.')[-1] == "weight":
                        weight_ = nerf_weights[layer_name].numpy().squeeze().transpose()
                        nn.initializer.Assign(weight_)(self.stem_net.layers[layer_id].weight)

                elif layer_name.split('.')[0] == "views_linears": # coresponds to self.color_head.layers[0]
                    # Assign bias
                    if layer_name.split('.')[-1] == "bias":
                        bias_ = nerf_weights[layer_name].numpy()
                        nn.initializer.Assign(bias_)(self.color_head.layers[0].bias)
                    # Assign weight
                    if layer_name.split('.')[-1] == "weight":
                        weight_ = nerf_weights[layer_name].numpy().squeeze().transpose()
                        nn.initializer.Assign(weight_)(self.color_head.layers[0].weight)

                elif layer_name.split('.')[0] == "feature_linear": # coresponds to self.feature_linear
                    # Assign bias
                    if layer_name.split('.')[-1] == "bias":
                        bias_ = nerf_weights[layer_name].numpy()
                    ...
                    if layer_name.split('.')[-1] == "weight":
                        weight_ = nerf_weights[layer_name].numpy().squeeze().transpose()
                        nn.initializer.Assign(weight_)(self.feature_linear.weight)

                elif layer_name.split('.')[0] == "alpha_linear": # coresponds to self.density_head
                    # Assign bias
                    if layer_name.split('.')[-1] == "bias":
                        bias_ = nerf_weights[layer_name].numpy()
                        nn.initializer.Assign(bias_)(self.density_head.layers[0].bias)
                    # Assign weight
                    if layer_name.split('.')[-1] == "weight":
                        weight_ = nerf_weights[layer_name].numpy().transpose()
                        nn.initializer.Assign(weight_)(self.density_head.layers[0].weight)

                elif layer_name.split('.')[0] == "rgb_linear": # coresponds to self.color_head.layers[0]
                    # Assign bias
                    if layer_name.split('.')[-1] == "bias":
                        bias_ = nerf_weights[layer_name].numpy()
                        nn.initializer.Assign(bias_)(self.color_head.layers[1].bias)
                    # Assign weight
                    if layer_name.split('.')[-1] == "weight":
                        weight_ = nerf_weights[layer_name].numpy().squeeze().transpose()
                        nn.initializer.Assign(weight_)(self.color_head.layers[1].weight)
                else:
                    raise Exception("Wrong torch weights.")

        
    def get_densities(self, ray_samples: RaySamples = None,
        ...
        dis_to_center = paddle.linalg.norm(pts, p=2, axis=-1, keepdim=True).clip(1.0, 1e10)
    ...
    def get_colors(self, ray_samples: RaySamples = None, 
...
class NeuSField(BaseField):
        ...
        if load_torch_data:
            ...
            for layer_name in sdf_weights.keys():
                ...
                if layer_name.split('.')[-1] == "bias":
                    ...
                    nn.initializer.Assign(bias_)(self.sdf_network.layers[layer_id].bias)
    
                # Assign weight_g
                if layer_name.split('.')[-1] == "weight_g":
                    weight_g_ = sdf_weights[layer_name].numpy().squeeze()
                    nn.initializer.Assign(weight_g_)(self.sdf_network.layers[layer_id].weight_g)
                
                # Assign weight_v
                if layer_name.split('.')[-1] == "weight_v":
                    weight_v_ = sdf_weights[layer_name].numpy().transpose()
                    nn.initializer.Assign(weight_v_)(self.sdf_network.layers[layer_id].weight_v)
            ...
            for layer_name in color_weights.keys():
                ...
                if layer_name.split('.')[-1] == "bias":
                    ...
                    nn.initializer.Assign(bias_)(self.color_network.layers[layer_id].bias)
    
                # Assign weight_g
                if layer_name.split('.')[-1] == "weight_g":
                    weight_g_ = color_weights[layer_name].numpy().squeeze()
                    nn.initializer.Assign(weight_g_)(self.color_network.layers[layer_id].weight_g)
                
                # Assign weight_v
                if layer_name.split('.')[-1] == "weight_v":
                    weight_v_ = color_weights[layer_name].numpy().transpose()
                    nn.initializer.Assign(weight_v_)(self.color_network.layers[layer_id].weight_v)                
    ...
    def get_sdf_output(self, ray_samples: RaySamples,
    ...
    def get_gradients(self, ray_samples: RaySamples,
    ...
    def get_colors(self, 
        ...
        rendering_input = paddle.concat([pts, dirs, gradients, sdf_features], axis=-1)
        color = self.color_network(rendering_input)

        return color
        
    def get_inside_filter_by_norm(self, ray_samples: RaySamples,
                                  which_pts: str = "mid_points") -> paddle.Tensor:
        if which_pts == "mid_points":
            pts = ray_samples.frustums.positions
        else:
    ...
    def get_inv_s(self,
                  batch_size: int,
                  n_samples: int):
        inv_s = self.deviation_network(paddle.zeros([1, 3]))[:, :1].clip(1e-6, 1e6)
...
class SingleVarianceNetwork(nn.Layer):
    def __init__(self, init_val,
                 load_torch_data: bool = True):
        super(SingleVarianceNetwork, self).__init__()
        variance = self.create_parameter(shape=[1], 
