...
class GeometricInit(object):
    ...
    def initialize_layer(self, layer, dim, which_layer):
        ...
        if which_layer == "last":
            ...
            _weight = np.random.normal(np.sqrt(np.pi) / np.sqrt(dim), 0.0001, _weight.shape)
            ...
            _weight = layer.weight.numpy()
            _weight = np.random.normal(0.0, np.sqrt(2) / np.sqrt(dim), _weight.shape)
            _weight[3:, :] = 0.0
            ...
            # Init weight
            _weight = layer.weight.numpy()
            _weight = np.random.normal(0.0, np.sqrt(2) / np.sqrt(out_dim), _weight.shape)
...
class MLP(nn.Layer):
    def __init__(self,
        ...
        if len(list(self.skip_layers)) > 0 and skip_connection_way == "concat_in":
        ...
        if isinstance(activation, str):
            ...
            if func_name == "softplus":
                ...
                self.activation = PADDLE_ACTIVATION[func_name.lower()](beta, thre)

        else:
            raise NotImplementedError("activation should be either str or list.")
        ...
        if not weight_init is None:
            ...
            self.weight_init_fn = PADDLE_INITIALIZATION[init_fn_name](weight_init[1:])
            if init_fn_name == "geometric_init":
                assert(len(self.layers) > 1)
                self.weight_init_fn.initialize(self.layers, 
                                               self.skip_layers,
    ...
    def _populate_layers(self) -> nn.LayerList:
        ...
        if self.num_layers == 1:
            layers.append(
                nn.Linear(self.input_dim, self.output_dim, bias_attr=self.with_bias))                
        else:
            # First layer
            layers.append(
                nn.Linear(
