...
__all__ = ["SpacedSampler", "UniformSampler", "DisparitySampler", "LogSampler", "NeuSOutsideSampler"]
...
class SpacedSampler(BaseSampler):
    ...
    def generate_ray_samples(self,
        ...
        euclidean_bins = spacing2euclidean_fn(bins)  # [num_rays, num_samples + 1]
...
class NeuSOutsideSampler(BaseSampler):
    ...
    def generate_ray_samples(self, ray_bundle: RayBundle, num_samples: int = None):
        num_samples = num_samples or self.num_samples
        ...
        euclidean_bins = fars / paddle.flip(bins, axis=-1) + self.inside_interval_len

        # Hard set positions
        origins = ray_bundle.origins.unsqueeze(-2).repeat_interleave(bins.shape[1], -2)
        directions = ray_bundle.directions.unsqueeze(-2).repeat_interleave(bins.shape[1], -2)
