...
class Frustums(object):
    ...
    def bin_points(self) -> paddle.Tensor:        
        origins = paddle.concat([self.origins, self.origins[:, -1, :].unsqueeze(axis=1)], axis=1)
        directions = paddle.concat([self.directions, self.directions[:, -1, :].unsqueeze(axis=1)], axis=1)
...
class RaySamples:
    ...
    def merge_samples(self, sample_list, ray_bundle=None, sample_dist=2./128., mode='force_concat'):        
        ...
        if mode == "sort": 
            ...
            for sample in sample_list:
                assert(self.spacing2euclidean_fn == sample.spacing2euclidean_fn)
            bins = self.spacing_bins[:, :-1].squeeze(-1)
            
            for i, sample in enumerate(sample_list):
                if not i == len(sample_list) - 1:
                    bins = paddle.concat([bins, sample.spacing_bins[:, :-1].squeeze(-1)], axis=-1)
                else:
                    bins = paddle.concat([bins, sample.spacing_bins.squeeze(-1)], axis=-1)
            ...
            euclidean_bins = paddle.concat([self.frustums.starts, self.frustums.ends[:, -1:, :]], axis=1)
            ...
            for i, sample in enumerate(sample_list):
                ...
                origins = paddle.concat([origins, sample.frustums.origins, origin_pad], axis=-2)

                direction_pad = sample.frustums.directions[:, -1:, :]
                directions = paddle.concat([directions, sample.frustums.directions, direction_pad], axis=-2)
                pixel_area = paddle.concat([pixel_area, sample.frustums.pixel_area], axis=-2)
                #camera_ids = paddle.concat([camera_ids, sample.camera_ids], axis=-1)
                camera_ids = paddle.concat([camera_ids, sample.camera_ids], axis=-2)
                sample_euclidean_bins = paddle.concat([sample.frustums.starts, sample.frustums.ends[:, -1:, :]], axis=1)
                euclidean_bins = paddle.concat([euclidean_bins, sample_euclidean_bins], axis=1)
            ...
            raise NotImplementedError("[mode] should be either 'force_concat' or 'sort'")
