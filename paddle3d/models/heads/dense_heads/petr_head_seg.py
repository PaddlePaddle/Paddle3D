...
class PETRHeadSeg(nn.Layer):
    ...
    def _init_layers(self):
        ...
        self.reference_points_lane = paddle.concat([xy[0].reshape([-1])[..., None],xy[1].reshape([-1])[..., None]], axis=-1)
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        outs_dec_lane, _ = self.transformer_lane(x, masks, query_lane, pos_embed, self.lane_branches)
    ...
    def export_forward(self, mlvl_feats, img_metas, time_stamp=None):  # need to be added lane part
        """Forward function.
        Args:
            mlvl_feats (tuple[Tensor]): Features from the upstream
                network, each is a 5D-tensor with shape
    ...
    def loss(self,
        ...
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_mask_i in zip(losses_cls[:-1], losses_bbox[:-1], losses_lane_masks[:-1]):
