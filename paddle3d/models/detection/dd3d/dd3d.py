...
class DD3D(nn.Layer):
    ...
    def __init__(self,
        ...
        self.register_buffer("pixel_mean", paddle.to_tensor(pixel_mean).reshape([1, -1, 1, 1]))
        self.register_buffer("pixel_std", paddle.to_tensor(pixel_std).reshape([1, -1, 1, 1]))
    ...
    def preprocess_image(self, x, size_divisibility):
        ...
        h_new = ((h_old + (size_divisibility - 1)) // size_divisibility) * size_divisibility
        w_new = ((w_old + (size_divisibility - 1)) // size_divisibility) * size_divisibility
        x = F.pad(x, [0, w_new-w_old, 0, h_new-h_old], value=0.0, mode='constant')
        return x
    
    def preprocess_box3d(self, box3d, intrinsic):
        box_pose = box3d[:, :, 0:4]
        tvec = box3d[:, :, 4:7]
        proj_ctr = paddle.mm(intrinsic.unsqueeze(1).tile([1, tvec.shape[1], 1, 1]), tvec.unsqueeze(-1)).squeeze(-1)
        proj_ctr = proj_ctr[:, :, :2] / proj_ctr[:, :, 2:]
        box3d_new = paddle.concat([box3d[:, :, 0:4], proj_ctr, box3d[:, :, 6:]], axis=2)
        return box3d_new

    def forward(self, samples):
        images = self.preprocess_image(samples["data"], self.size_divisibility)
        samples["bboxes_3d"] = self.preprocess_box3d(samples["bboxes_3d"], samples['meta']['camera_intrinsic'])
        ...
        if self.training:
            ...
            training_targets = self.prepare_targets(locations, samples["bboxes_2d"], samples["bboxes_3d"], samples["labels"], feature_shapes)
            ...
            for i, pred_instance in enumerate(pred_instances):
                ...
                for key in pred_instance[0].keys():
                    sum_c = sum([pred_instance[j][key].shape[0] for j in range(len(pred_instance))])
                    if sum_c==0:
                        pred_instance_cat[key] = pred_instance[0][key]
                    else:
                        pred_instance_cat[key] = paddle.concat([pred_instance[j][key] for j in range(len(pred_instance))], 0)
                pred_instances_cat.append(pred_instance_cat)

            # 2D NMS and pick top-K.
            if self.do_nms:
                pred_instances = self.fcos2d_inference.nms_and_top_k(pred_instances_cat, score_key)
    ...
    def compute_locations(self, features):
        ...
        for level, feature in enumerate(features):
            ...
            locations_per_level = self.compute_features_locations(
                h, w, self.input_strides[level], feature.dtype, offset=self.feature_locations_offset
            )
            locations.append(locations_per_level)
        return locations
    
    def compute_features_locations(self, h, w, stride, dtype='float32', offset="none"):
    ...
    def resize_instances(self, pred_boxes, height, width, image_size):
        ...
        scale_x, scale_y = (
                            width.cast('float32') / image_size[1],
                            height.cast('float32') / image_size[0]
                            )
        pred_boxes[:, 0::2] *= scale_x
        pred_boxes[:, 1::2] *= scale_y
        pred_boxes[:, 0::2] = pred_boxes[:, 0::2].clip(min=0.0, max=image_size[1])
        pred_boxes[:, 1::2] = pred_boxes[:, 1::2].clip(min=0.0, max=image_size[0])
        return pred_boxes
    ...
    def post_process(self, pred_instances, samples):
        ...
        for i, results_per_image in enumerate(pred_instances):
            ...
            bboxes_2d = self.resize_instances(results_per_image['pred_boxes'], height, width, samples["data"].shape[-2:])
            bboxes_3d = []
            alpha = []
            for j in range(results_per_image['pred_boxes3d'].shape[0]):
                bbox_3d, alpha_ = self.convert_3d_box_to_kitti(results_per_image['pred_boxes3d'][j:j+1, :], samples['meta']['camera_intrinsic'][i:i+1, ...].inverse())
    ...
    def convert_3d_box_to_kitti(self, boxes3d, inv_intrinsics):
        ...
        if quat.axis[2] > 0:
            v = self.pose(wxyz=Quaternion(axis=[0, 1, 0], radians=-quat.angle), tvec=tvec, v_=v_)
            rot_y = -quat.angle  
        else:
            v = self.pose(wxyz=Quaternion(axis=[0, 1, 0], radians=quat.angle), tvec=tvec, v_=v_)
            rot_y = quat.angle
