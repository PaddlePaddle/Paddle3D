...
class DD3DTargetPreparer(nn.Layer):
    ...
    """
    def __init__(self, input_strides, num_classes=5, center_sample=True, 
                radius=1.5, dd3d_on=True, sizes_of_interest=[64, 128, 256, 512]):
    ...
    def forward(self, locations, bboxes_2d, bboxes_3d, labels, feature_shapes):
        ...
        for l, loc_per_level in enumerate(locations):
            loc_to_size_range_per_level = paddle.to_tensor(self.sizes_of_interest[l], dtype=loc_per_level.dtype)
        ...
        training_targets = self.compute_targets_for_locations(locations, bboxes_2d, bboxes_3d, labels, loc_to_size_range, num_loc_list)

        training_targets["locations"] = [locations.clone() for _ in range(bboxes_2d.shape[0])]
        training_targets["im_inds"] = [
            paddle.ones([locations.shape[0]], dtype='int64') * i for i in range(bboxes_2d.shape[0])
        ]

        box2d = training_targets.pop("box2d", None)

        # transpose im first training_targets to level first ones
        training_targets = {k: self._transpose(k, v, num_loc_list) for k, v in training_targets.items() if k != "box2d"}

        training_targets["fpn_levels"] = [
            paddle.ones([len(loc)], dtype='int64') * level for level, loc in enumerate(training_targets["locations"])
        ]

        # Flatten targets: (L x B x H x W, TARGET_SIZE)
        labels = paddle.concat([x.reshape([-1]) for x in training_targets["labels"]], axis=0)
        box2d_reg_targets = paddle.concat([x.reshape([-1, 4]) for x in training_targets["box2d_reg"]], axis=0)

        target_inds = paddle.concat([x.reshape([-1]) for x in training_targets["target_inds"]], axis=0)
        locations = paddle.concat([x.reshape([-1, 2]) for x in training_targets["locations"]], axis=0)
        im_inds = paddle.concat([x.reshape([-1]) for x in training_targets["im_inds"]], axis=0)
        fpn_levels = paddle.concat([x.reshape([-1]) for x in training_targets["fpn_levels"]], axis=0)
        ...
        if self.dd3d_enabled:
            box3d_targets = paddle.concat([x.reshape([-1, 10]) for x in training_targets["box3d"]], axis=0)
            ...
            if box2d is not None:
                ...
                for lvl, per_lvl_box2d in enumerate(zip(*box2d)):
                    ...
                    batched_box2d_lvl = paddle.stack([x.T.reshape([4, h, w]) for x in per_lvl_box2d], axis=0)
                    batched_box2d.append(batched_box2d_lvl)
                targets.update({"batched_box2d": batched_box2d})

        return targets

    def compute_targets_for_locations(self, locations, bboxes_2d, bboxes_3d, labels_batch, size_ranges, num_loc_list):
        # targets
        labels = []
        box2d_reg = []
        ...
        for im_i in range(bboxes_2d.shape[0]):
            ...
            # no gt
            if bboxes.numel() == 0:
                labels.append(paddle.zeros([locations.shape[0]]) + self.num_classes)
            ...
            indes = paddle.stack([paddle.arange(len(locations)), locations_to_gt_inds], 1)
    ...
    def get_sample_region(self, boxes, num_loc_list, loc_xs, loc_ys):
        ...
        for level, num_loc in enumerate(num_loc_list):
            ...
            center_gt[beg:end, :, 0] = paddle.where(xmin > boxes[beg:end, :, 0], xmin, boxes[beg:end, :, 0])
            center_gt[beg:end, :, 1] = paddle.where(ymin > boxes[beg:end, :, 1], ymin, boxes[beg:end, :, 1])
            center_gt[beg:end, :, 2] = paddle.where(xmax > boxes[beg:end, :, 2], boxes[beg:end, :, 2], xmax)
            center_gt[beg:end, :, 3] = paddle.where(ymax > boxes[beg:end, :, 3], boxes[beg:end, :, 3], ymax)
            beg = end
    ...
    def _transpose(self, k, training_targets, num_loc_list):
        ...
        if k=="box3d":
            for im_i in range(len(training_targets)):
                ...
                training_targets[im_i] = training_targets[im_i].split(num_loc_list, axis=0)
