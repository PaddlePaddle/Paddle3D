...
def unproject_points2d(points2d, inv_K, scale=1.0):
    ...
    unprojected = paddle.matmul(inv_K, points2d)  # (N, 3, 3) x (N, 3, 1) -> (N, 3, 1)
...
class DisentangledBox3DLoss(nn.Layer):
    ...
    def forward(self, box3d_pred, box3d_targets, locations, inv_intrinsics, weights=None):

        box3d_pred = box3d_pred.cast('float32')
        box3d_targets = box3d_targets.cast('float32')

        target_corners = self.corners(box3d_targets[..., :4], box3d_targets[..., 4:6], box3d_targets[..., 6:7], box3d_targets[..., 7:], inv_intrinsics)

        disentangled_losses = {}
        index = [0, 4, 6, 7, 10]
        for i, component_key in enumerate(["quat", "proj_ctr", "depth", "size"]):
            disentangled_boxes = box3d_targets.clone()
            disentangled_boxes[..., index[i]:index[i+1]] = box3d_pred[..., index[i]:index[i+1]]
            pred_corners = self.corners(disentangled_boxes[..., :4], disentangled_boxes[..., 4:6], disentangled_boxes[..., 6:7], disentangled_boxes[..., 7:], inv_intrinsics)
        ...
        pred_corners = self.corners(box3d_pred[..., :4], box3d_pred[..., 4:6], box3d_pred[..., 6:7], box3d_pred[..., 7:], inv_intrinsics)
        entangled_l1_dist = (target_corners - pred_corners).detach().abs().reshape([-1, 24]).mean(axis=1)
    ...
    def corners(self, quat, proj_ctr, depth, size, inv_intrinsics):
        ...
        rotation = t3d.Rotate(R=R.transpose([0, 2, 1]))  # Need to transpose to make it work.

        tfm = rotation.compose(translation)

        _corners = 0.5 * paddle.to_tensor(BOX3D_CORNER_MAPPING).T
        lwh = paddle.stack([size[:, 1], size[:, 0], size[:, 2]], -1) # wlh -> lwh
