...
class Ego_emb(nn.Layer):
    ...
    def get_curlidar2prevlidar(self, img_metas, x):
        ...
        '''
        curlidar2prevcam = paddle.to_tensor(img_metas[0]['extrinsics'][6].T, dtype='float32') # (4, 4)
        curlidar2curcam = paddle.to_tensor(img_metas[0]['extrinsics'][0].T, dtype='float32')  # (4, 4)
...
class CrossAttention(nn.Layer):
    def __init__(self, dim, heads, dim_head, qkv_bias, norm=nn.LayerNorm):
        ...
        self.proj_dict = nn.LayerDict({
            ...
            'k_g': nn.Linear(dim, heads * dim_head, bias_attr=qkv_bias),
            'q_a': nn.Linear(dim, heads * dim_head, bias_attr=qkv_bias),
            'k_a': nn.Linear(dim, heads * dim_head, bias_attr=qkv_bias),
            'v':nn.Linear(dim, heads * dim_head, bias_attr=qkv_bias)
        })
    ...
    def forward(self, k_g, q_g, k_a, q_a, v, mask):
        ...
        dot_g = dot_g * self.scale_g.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).unsqueeze(0)
...
class CAPETransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        image_plane = paddle.stack([xs, ys, paddle.ones_like(xs)], axis=-1).flatten(0, 1).astype('float32') # hw * 3
        self.register_buffer('image_plane', image_plane, persistable=True)
        
        self.register_buffer('bound', paddle.to_tensor(bound).reshape([2, 3]), persistable=True)

        self.reference_points = nn.Embedding(self.num_queries, 3)
        param_init.uniform_init(self.reference_points.weight, 0, 1)

        if self.with_time:
            self.mf = nn.LayerList([MLP_Fusion(tf_layer['hidden_dim']) for _ in range(num_layers)])

        self.cva_layers = nn.LayerList([copy.deepcopy(att_layer) for _ in range(num_layers)])
        ...
        self.register_buffer('cam_bound', paddle.to_tensor(self.cam_position_range).reshape([2, 3]), persistable=True)
    ...
    def forward(self, feature, mask, I_inv, R_inv, t, img_metas, return_prev_query=False, mean_time_stamp=0.5):
        return_list = []
        return_prev_list = []
        bs, nc, d, h, w = feature.shape#[:2]
        ...
        if self.with_time:
            cur_x = x
            prev_x = x
            lidar_obj_pe = paddle.concat([lidar_obj_pe, lidar_obj_pe], 0)   # 2 * bs, N, 256
            ...
            if not return_prev_query:
                ...
                return paddle.stack(return_list), paddle.stack(return_prev_list), ref_points, mask_dict
        else:
            feature, mask, img_embed, bev_embed, R = self.prepare_emb(feature, mask, I_inv, R_inv, t, ref_points, img_metas, f)
...
class CrossViewAttention(nn.Layer):
    ...
    def forward(self,
        ...
        if self.training:
            updated_x = recompute(self.cross_attend, img_embed, bev_embed, k_a, q_a, val, mask)
        else:
