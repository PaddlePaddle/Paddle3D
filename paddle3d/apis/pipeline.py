...
def validate_grad(model: paddle.nn.Layer):
    """
    Validate gradient of all trainable parameters.
    """
    empty_grad_name = []
    ...
    if len(empty_grad_name) > 0:
        raise ValueError(
            'No gradient found in these parameters which require grad: {}'.
            format(empty_grad_name))
