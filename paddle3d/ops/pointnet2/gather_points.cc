...
std::vector<paddle::Tensor> gather_points_cuda_forward(
    ...
    auto out_tensor = paddle::empty({b, c, npoints}, paddle::DataType::FLOAT32, paddle::GPUPlace());
...
std::vector<paddle::Tensor> gather_points_cuda_backwarad(
    ...
    gather_points_grad_cuda_launcher(b, c, n, npoints, grad_out, idx, grad_points);
...
// data type infer
std::vector<paddle::DataType> GatherInferDtype(
