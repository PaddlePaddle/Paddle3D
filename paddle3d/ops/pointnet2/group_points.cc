...
std::vector<paddle::Tensor> group_points_cuda_forward(
    ...
    auto out_tensor = paddle::empty({b, c, npoints, nsample}, paddle::DataType::FLOAT32, paddle::GPUPlace());
...
std::vector<paddle::Tensor> group_points_cuda_backward(
    ...
    auto *idx = idx_tensor.data<int>();
    auto grad_points_tensor = paddle::full({b, c, n}, 0.0, paddle::DataType::FLOAT32, paddle::GPUPlace());
    auto *grad_points = grad_points_tensor.data<float>();

    group_points_grad_cuda_launcher(b, c, n, npoints, nsample, grad_out, idx, grad_points);
