...
void points_in_boxes_cuda_launcher(const int batch_size, const int boxes_num, const int pts_num, const float *boxes,
    const float *pts, int *box_idx_of_points);

// op forward
std::vector<paddle::Tensor> points_in_boxes_cuda_forward(
    const paddle::Tensor &pts_tensor,
    const paddle::Tensor &boxes_tensor
) {
    ...
    auto box_idx_of_points_tensor = paddle::full({batch_size, pts_num}, -1, paddle::DataType::INT32, paddle::GPUPlace());
    auto *boxes = boxes_tensor.data<float>();
    auto *pts = pts_tensor.data<float>();
    auto *box_idx_of_points = box_idx_of_points_tensor.data<int>();

    points_in_boxes_cuda_launcher(batch_size, boxes_num, pts_num, boxes, pts, box_idx_of_points);
...
// dtype infer
std::vector<paddle::DataType> PtInBoxInferDtype(
